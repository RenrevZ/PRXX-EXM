<template>
    <AuthenticatedLayout :paramsID="paramsID">
...
const { getAllProductWPhoto } = useProductStore()

onMounted(async () => {
    await getAllProductWPhoto(props.paramsID)
    
    inputs.productName = product.value.name
    inputs.categories = product.value.category
    inputs.description = product.value.description
    dateAndTime.value = product.value.date+'T'+product.value.time
})
...
const SubmitAllForms = async () =>{
    if(dateAndTime.value === ''){
        ...
        try {
           UpdateProduct(formData,props.paramsID)
        } catch (error) {
